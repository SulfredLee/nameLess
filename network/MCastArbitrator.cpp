...
MCastArbitrator::~MCastArbitrator()
{
    ...
    m_ArbiSoc.LeaveGroup(ARBIGROUP);
}
    ...
    while(m_runSend)
    {
        switch(m_status)
        {
            ...
            case MCArbiStatus::RAISEHAND:
                m_ArbiSoc.Send(ARBIGROUP, &fingerPrintData[0], totalLen);
                break;
...
void MCastArbitrator::ReceiveArbitration()
{
    ...
    while(m_runReceive)
    {
        if (m_ArbiSoc.SelectRead(500000, 0) == Multicast::MCStatus::READ) // wait for 500 milliseconds
        {
            if (m_ArbiSoc.Recv(receivedData, fromAddress, fromPort, byteRecv) == Multicast::MCStatus::SUCCESS)
            {
                int dataType;
                short arbitratorID;
                short instance;
...
void MCastArbitrator::PrintStatus()
{
    if (m_preStatus.load() != m_status.load())
    {
        ...
        LOGMSG_INFO("Status updated %s", GetStatusString().c_str());
    }
}
