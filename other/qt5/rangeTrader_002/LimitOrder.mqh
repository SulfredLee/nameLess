...
void NormalizeLimitOrder(LimitOrder& order)
{
    order.m_price = NormalizeDouble(order.m_price, Digits());
    order.m_TP = NormalizeDouble(order.m_TP, Digits());
    order.m_SL = NormalizeDouble(order.m_SL, Digits());
}
