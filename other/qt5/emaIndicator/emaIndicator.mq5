...
input int g_LRange_SPeriod = 8;
input int g_LRange_LPeriod = 21;
...
void MapIndicator()
{
    ...
    SetIndexBuffer(3, g_EMA_LRange_SPeriod, INDICATOR_DATA);
    SetIndexBuffer(4, g_EMA_LRange_LPeriod, INDICATOR_DATA);
    for (int i = 0; i < 5; i++)
    {
                ...
                const double &price[])
{
    if (prev_calculated == 0)
    {
        ...
        g_EMA_LRange_SPeriod_Len = ArrayInitialize(g_EMA_LRange_SPeriod, EMPTY_VALUE);
        g_EMA_LRange_LPeriod_Len = ArrayInitialize(g_EMA_LRange_LPeriod, EMPTY_VALUE);
    }
    ...
    CopyBuffer(g_hEMA_LRange_LPeriod, 0, 0, g_EMA_LRange_len, g_EMA_LRange_LPeriod);
    CopyBuffer(g_hEMA_LRange_SPeriod, 0, 0, g_EMA_LRange_len, g_EMA_LRange_SPeriod);
...
void OnDeinit(const int reason)
{
    ...
    ArrayFree(g_EMA_LRange_SPeriod);
    ArrayFree(g_EMA_LRange_LPeriod);
    IndicatorRelease(g_hEMA_SRange_LPeriod);
    IndicatorRelease(g_hEMA_SRange_MPeriod);
    IndicatorRelease(g_hEMA_SRange_SPeriod);
    IndicatorRelease(g_hEMA_LRange_LPeriod);
    IndicatorRelease(g_hEMA_LRange_SPeriod);
}
//+------------------------------------------------------------------+
