...
{
    NLTimeUTC curTime;
    LOGMSG_DBG_S() << "curTime: " << curTime.toString("%Y %m %d %H %M %S") << "\n";
    LOGMSG_DBG_S() << "m_DateTime: " << m_DateTime.toString("%Y %m %d %H %M %S") << "\n";
...
void DailyTimer::Main()
{
    ValidateTargetTime();
    {
        std::stringstream ss; ss << "Next alert time: " << m_DateTime.toString("%Y %m %d %H %M %S");
        LOGMSG_MSG("%s\n", ss.str().c_str());
    }
